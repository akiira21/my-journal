...
) RETURNING id, slug, title, description, content_url, categories, tags, featured, view_count, read_time_minutes, created_at, updated_at, published_at, is_archived, cover_url, search_vector
`
...
SELECT id, slug, title, description, content_url, categories, tags, featured, view_count, read_time_minutes, created_at, updated_at, published_at, is_archived, cover_url, search_vector FROM posts
WHERE id = $1
`
...
func (q *Queries) SearchPosts(ctx context.Context, arg SearchPostsParams) ([]SearchPostsRow, error) {
	rows, err := q.db.Query(ctx, searchPosts, arg.PlaintoTsquery, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
