# RNN Sequential Processing Limitation Animation
# Run with: python -m manim -pql rnn_sequential_limitation.py RNNSequentialComputation
...
class RNNSequentialComputation(Scene):
    def construct(self):
        # Title emphasizing the limitation
        ...
        # Setup the RNN architecture (centered)
        self.setup_rnn_architecture()
        
        # Show sequential computation step by step
        self.demonstrate_sequential_processing()
        
        # Show the complete sequence and emphasize limitations
        self.show_complete_sequence()
    
    def setup_rnn_architecture(self):
        """Setup the basic RNN architecture components - centered on screen"""
        
        # Input sequence (4 time steps)
        self.sequence_length = 4
        self.input_texts = ["Hello", "how", "are", "you"]
        
        # Create input boxes (centered)
        ...
        for i, text in enumerate(self.input_texts):
            ...
            input_item.shift(LEFT * 3 + RIGHT * i * 2)  # Centered positioning
        ...
        for i in range(self.sequence_length):
            ...
            hidden_state.shift(LEFT * 3 + RIGHT * i * 2 + UP * 1.5)
            self.hidden_states.add(hidden_state)
        
        # Output states (centered)
        self.outputs = VGroup()
        for i in range(self.sequence_length):
            ...
            output.shift(LEFT * 3 + RIGHT * i * 2 + UP * 3)
            self.outputs.add(output)
        
        # Weight matrices (positioned relative to centered architecture)
        self.W_xh = Rectangle(width=1, height=0.6, color=PURPLE, fill_opacity=0.4)
        self.W_xh_label = Text("Wxh", font_size=16, color=WHITE)
        self.W_xh_group = VGroup(self.W_xh, self.W_xh_label)
        self.W_xh_group.shift(LEFT * 4.5 + UP * 0.75)
        
        self.W_hh = Rectangle(width=1, height=0.6, color=ORANGE, fill_opacity=0.4)
        self.W_hh_label = Text("Whh", font_size=16, color=WHITE)
        self.W_hh_group = VGroup(self.W_hh, self.W_hh_label)
        self.W_hh_group.shift(LEFT * 2.5 + UP * 0.75)
        
        self.W_hy = Rectangle(width=1, height=0.6, color=PINK, fill_opacity=0.4)
        self.W_hy_label = Text("Why", font_size=16, color=WHITE)
        self.W_hy_group = VGroup(self.W_hy, self.W_hy_label)
        self.W_hy_group.shift(LEFT * 4.5 + UP * 2.25)
        ...
        self.play(
            Create(self.W_xh_group),
            Create(self.W_hh_group),
            Create(self.W_hy_group)
        )
        self.wait(1)
    ...
    def process_time_step(self, t, prev_h=None):
        """Process a single time step - simplified without detailed computation boxes"""
        ...
        # Arrow from input to hidden state
    ...
    def show_complete_sequence(self):
        """Show the complete sequence processing and emphasize limitations"""
        
        # Create a summary view emphasizing the problem
        ...
        # Final summary emphasizing problems
        final_text = VGroup(
            Text("Major Limitations:", font_size=24, color=RED),
            Text("• Sequential dependency - can't parallelize", font_size=20, color=WHITE),
            Text("• Information bottleneck through hidden states", font_size=20, color=WHITE),
            Text("• Vanishing gradient problem over long sequences", font_size=20, color=WHITE),
            Text("• Slow training due to sequential nature", font_size=20, color=WHITE)
        )
        final_text.arrange(DOWN, buff=0.3, aligned_edge=LEFT)
        final_text.to_edge(DOWN, buff=1)
        
        self.play(
            LaggedStart(
                *[Write(text) for text in final_text],
                lag_ratio=0.5
            )
        )
        self.wait(3)
    ...
    def animate_information_flow(self):
        """Animate information flowing through the network to show sequential nature"""
        
        for cycle in range(2):
            # Flow through inputs sequentially
            for t in range(self.sequence_length):
                self.play(
...
if __name__ == "__main__":
    ...
    Low quality (fast): python -m manim -pql rnn_sequential_limitation.py RNNSequentialComputation
    Medium quality: python -m manim -pqm rnn_sequential_limitation.py RNNSequentialComputation  
    High quality: python -m manim -pqh rnn_sequential_limitation.py RNNSequentialComputation
    """
    pass
